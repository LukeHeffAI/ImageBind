...
for i in range(5):
    ...
    inputs[i] = {
        ...
        ModalityType.AUDIO: data.load_and_transform_audio_data(audio_list[i*400:(i+1)*400-1], device),
    }
...
elif text_list == text_list_esc50:
...
for i in range(len(embeddings)):
    ...
    model_output = torch.softmax(embeddings[ModalityType.AUDIO][1] @ embeddings[ModalityType.TEXT].T, dim=-1)
    model_output_concat = torch.cat(model_output_concat, model_output)

model_output = model_output_concat
...
print(model_output_numbers[1:10])
